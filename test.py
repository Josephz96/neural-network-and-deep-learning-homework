...
def load_modules_parameter(net, load_path, file_name = 'joseph_module_param.npy'):
...
class Net:
    ...
    def param_plot(self):
        ...
        for index in range(W1_plot.shape[0]):
            plt.subplot(13, 10, index+1)
            plt.imshow(W1_plot[index, :, :], cmap=plt.cm.gray)
            plt.axis('off')
            plt.savefig('D:/博士课程/神经网络与深度学习/nnh/plot/MLP_W2.png')
...
if __name__ == '__main__':
    MLP_Net = Net()
    load_path = 'D:/博士课程/神经网络与深度学习/nnh/model_param/'
